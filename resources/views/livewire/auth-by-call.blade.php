    ...
    <div class="content">
                ...
                <div class="signReg__form">
                    <form {{ $submit_method }}>
                        ...
                        @else
                            ...
                            <button type="submit" class="sendsms">Входящий звонок<span></span></button>
                        @endif
        ...
        @else
            <div class="number" id="numberBlock" style="display: block !important; position: relative; transform: translateY(0%); top: 25px; left: 0; margin: 0 auto" >
                <h2 class="number__title">Введите цифры.</h2>
                <div class="back swiper-button-prev" wire:click.prevent="$emit('back')"></div>
                <p class="number__description">Мы совершаем звонок на номер<span class="number__num">{{ Session('user')['phone'] }}</span></p>
{{--                {{dd($phone)}}--}}
                @if(session()->has('callback'))
                    <span class="error"> {{ session('callback')['message'] }}</span>
                    <div class="callback">  {!! session('callback')['phone'] !!} </div>
                @else
                    <div class="number__text">Введите 4 последние цифры номера с которого был звонок</div>
                    <form id="accept">
                        @csrf
                        <input name="code" type="text" class="number__input" wire:model="code" wire:keyup="getCode()" placeholder="0000" >
                        @if(session()->has('error_code')) <div><span class="error"> {{ session('error_code') }}</span></div>@endif
                        @if(session()->has('allow_code')) <div><span class="alert-success"> {{ session('allow_code') }}</span></div>@endif
                    </form>
                    <div class="number__new">
                        Заказать повторный звонок можно через 0:<div class="number__time">90</div>
                    </div>
{{--                    <div class="number__again">--}}
{{--                        <a href="" wire:click.prevent="$emit('resendCode')">Повторный звонок</a>--}}
{{--                    </div>--}}
                    <script>
                        timer();
                    </script>
