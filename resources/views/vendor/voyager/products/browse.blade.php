                        ...
                        @if ($isServerSide)
                            <form method="get" class="form-search">
                                ...
                                <div id="search-input">
                                    ...
                                    <div class="col-2">
                                        <select id="search_input1" name="direction_id">
                                            @foreach($allDirection as $directions)
                                                <option value="{{$directions->id}}" {{$direction_id == $directions->id ? ' selected ': ' '}}>{{$directions->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    {{--     Фильтр по городам--}}
                                    <div class="col-2">
                                        <select id="search_input2" name="sity_name">
                                            <option value="-1" {{$partner_id == 0 ? ' selected ': ''}}>Выбирите партнера</option>
                                            @foreach($allStores as $stores)
                                                <option value="{{$stores->id}}"  {{$partner_id == $stores->partner_id ? ' selected ': ''}}>{{$stores->real_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    {{--       end фальтр по городам--}}
                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-hover">
                                ...
                                @foreach($dataTypeContent as $data)
                                        ...
                                        <tr>
                                            <td class="no-sort no-click bread-actions" width="50">
                                                @foreach($actions as $action)
                                                    @if (!method_exists($action, 'massAction'))
