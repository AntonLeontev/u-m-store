...
use App\Http\Controllers\Voyager\Partners\PartnersController;
use App\Jobs\ChainApi\generateWallet;
use App\Jobs\ChainApi\MarketplaceRegisterSeller;
...
class PartnerFormRegistration extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $dataTypeContent = $dataTypeContent->sortByDesc('status');
    ...
    // POST BR(E)AD
    public function createNewPartner(Request $request, $city_id, $direction_id)
    {
        $partner = Partners::create([
            ...
            'organisation_name'=>$request->shop_name,
            'org_full_name'=>$request->org_full_name,
            'partner_name'=>$request->org_short_name,
            'director_name'=>$request->director_name,
            'bohalter_name'=>$request->bohalter_name,
            'telephone'=>$request->org_tel,
            'mobile_tel_owner'=>$request->mobile_tel_owner,
            'email'=>$request->email,
            ...
            'inn'=>$request->inn,
            'ogrn'=>$request->ogrn,
            'ogrn_ip'=>$request->ogrn_ip,
            'bik'=>$request->bik,
            'kor_account'=>$request->kor_account,
            ...
            'who_gave'=>$request->who_gave

        ]);
        if ($partner) {
            if($request->user_id){
                UmHelp::updatePartnerIdInUsers($request->user_id,$partner->id);
            }
            ...
            Store::find($city_id)->update(['partner_id'=>$partner->id]);
        } return $partner->id;
    }
...
//        dd($request);
        if ($request->status == 'VALIDATE')
        {
            ...
            $user_id = $request->user_id;
            //
            $user_wallet = UserWallet::firstWhere('user_id', $user_id);

            //Присваиваем кошельку партнера статус saler и устанавливаем наценку.
            if($user_id > 0 && $user_wallet!=null)
            {
                if($request->partner_type =='Ген. Партнер') $markup = 15;
                else $markup = 25;

                //Запуск задания в фоне
                MarketplaceRegisterSeller::dispatch($user_wallet->wallet_address, $markup);

                //Изменяем статус у кошелька.
                $user_wallet->wallet_type = 'buyer&seller';
                $user_wallet->save();

            } elseif ($user_id!=null && $user_wallet==null)
            {
                  dd('Ошибка. У данного партнера нет сгенерированного кошелька. Нужно произвести ручную генерацию.');
//                Bus::chain([
//                    new generateWallet::dispatch($attr['password'], Auth::user());
//                    new MarketplaceRegisterSeller($user_wallet->wallet_address)
//                ])->dispatch();

            }else{
                dd('Ошибка. Не указан пользователь для данной записи!');
            }
//
            $city = Store::where('real_name',$request->city_name)->first();
            $direction = Directions::where('name',$request->direction)->first();

            if(is_null($city))
            {
                return redirect()->route('add.city' ,['city_name'=>$request->city_name]);

            }
            elseif(is_null($direction))
            {
                echo 'Нет такого вида услуг или товаров в таблице "Направления". Добавьте эту услугу потом измените статус партнера.';
                dd("Ошибка статус партнера не изменен!");
            }
            else {
                $partner_id = $this->createNewPartner($request, $city->id, $direction->id);
                $request->merge(['status' => 'ADD']);
                $request_array = $request->only('city_name','partner_type','direction','email', 'mobile_tel_owner');
                $request_array['id в базе'] = $partner_id;
                UmHelp::sendTelegramToManager($request_array, 'СОЗДАН НОВЫЙ ПАРТНЕР!');
