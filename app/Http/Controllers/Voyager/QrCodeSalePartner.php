...
class QrCodeSalePartner extends VoyagerBaseController
{
    ...
    public function addFullUrl(Request $request)
    {
        if($request->store_id>0 and $request->qr_slug);
        {
            if($city_slug = Store::find($request->store_id)) {

                $city_slug = $city_slug->slug;

                $full_url = 'https://unitedmarket.org/' . $city_slug . '/catalog/flowers?qr=' . $request->qr_slug;
                $request->request->add(['full_url' => $full_url]);
            }
            else $request->request->add(['full_url' => '']);
    ...
    {
        $request = $this->addFullUrl($request);
//        if($request->store_id and $request->qr_slug);
//        {
//            $city_slug = Store::find($request->store_id)->slug;
//            $full_url = 'https://unitedmarket.org/'.$city_slug.'/catalog/flowers?qr='.$request->qr_slug;
//            $request->request->add(['full_url' => $full_url]);
//
//
