...
namespace App\Http\Controllers\Voyager\Store;
...
//        Спрятать ненужные поля для данного типа партнера если он есть.
        if ($dataTypeContent->partner_type)
        {
            $this->hidenFildsForType($dataType, $dataTypeContent );
        }
    ...
    /**Custom function
     * Функция скрывает поля для разных типов партнеров.
     */
    public function hidenFildsForType(& $dataType, $dataTypeContent)
    {
        if ($dataTypeContent->partner_type == 'ИП') {
            $hiden_value = [
                'fio',
                'passport_data',
                'reg_address',
        ...
        } elseif ($dataTypeContent->partner_type == 'ООО') {
            $hiden_value = [
                'fio',
                'passport_data',
                'reg_address',
        ...
        } elseif ($dataTypeContent->partner_type == 'Самозанятый') {
        ...
        foreach ($dataType->editRows as $row) {
            if ($row->field == 'partner_type') {
                foreach ($dataType->editRows as $key => $row) {
                    if (in_array($row->field, $hiden_value)) {
     ...
     */
    public function updatePartnerIdInUsers($user_id, $partner_id)
    {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->user_id) {
            UmHelp::updatePartnerIdInUsers($request->user_id, $id);
//            $this->updatePartnerIdInUsers();
        }

        //Отслеживаем изменение наценки партнера
        $markup_new = $request->markup;
        $markup_old = Partners::find($request->id)->markup;

        //Если наценка изменилась то обновляем цены в таблицах product_option_value и product_to_stores у этого
        // партера по partner_id

        if ($markup_new != $markup_old) ;
        {
//            Изменение цен в таблицах ProductOptionValue и ProductToStore и округление до 99 методом UmHelp::salePrice

            $product_option_values = ProductOptionValue::where('partner_id', $request->id)->get();
//            dd($product_option_values);
            foreach ($product_option_values as $product_option_value) {
                $product_option_value->price = UmHelp::salePrice($product_option_value->partner_price *
                    ($markup_new / 100 + 1));
                $product_option_value->save();
            }
            $products_to_stores = Product_to_store::where('partner_id', $request->id)->get();

            foreach ($products_to_stores as $product_to_store) {
                $product_to_store->store_price = UmHelp::salePrice($product_to_store->partner_price * ($markup_new / 100 + 1));
                $product_to_store->save();
            }
//            DB::update('update product_option_values set price = partner_price * ? where partner_id = ?',
//                [$markup_new/100+1,
//                $request->id]);
//            DB::update('update product_to_stores set store_price = partner_price * ? where partner_id = ?',
//                [$markup_new/100+1,
//                    $request->id]);
        }
    ...
    public function store(Request $request)
    {
		...
		DB::table('stores')->where('id', $data->id)->update([
			'partner_id' => $request->get('partner_id'),
			'direction_id' => $request->get('direction_id'),
		]);
		DB::table('partners')->where('id', $request->get('partner_id'))->update([
			'store_id' => $data->id,
		]);
