...
class SiteCloneShopSliderSettings extends Component
{
    use WithFileUploads;
...
//        if($propertyName=='color') dd($propertyValue);

//        dd($this->uploaded_banners);
    }

    public function getAllBanners()
    {
        return CloneSliders::where('partner_id', Auth::user()->partner_id)->get();
    }
...
//        $this->emit('uploaded_banner_remove');
//        redirect()->route('admin.shop.slider');
//        dd($this->uploaded_banners);
    }
...
//        $partner = Partners::find(Auth::user()->partner_id);
        if ($this->banner_image->extension()) {
            ...
            $imageName = $this->banner_image->storeAs('SiteClone/Banners' . Carbon::now()->format('FY'), $imageName);
//            dd($imageName);
